...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, zerooned):
        golist = zerooned.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_zerooned(zerooned, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_zerooned(self, zerooned, rec):
        import decimal
        import zeroonelib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from zerooned...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from zerooned, with every run
        ...
        # get/create, then sync payment amounts, etc. from zerooned - zerooned is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(zerooned)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from zerooned! %s" % e)
            govobj.vote_delete(zerooned)
            return (govobj, None)
    ...
    def vote_delete(self, zerooned):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(zerooned, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, zerooned, signal, outcome):
        import zeroonelib
        ...
        output = zerooned.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = zeroonelib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(zerooned, signal)

    def sync_network_vote(self, zerooned, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = zerooned.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = ZEROONED_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 zeroone addr, non-multisig
            if not zeroonelib.is_valid_zeroone_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid ZOC address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # ZeroOneCore restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (zeroonelib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = ZEROONED_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not zeroonelib.is_valid_zeroone_address(addr, config.network):
    ...
    def hash(self):
        import zeroonelib
        return zeroonelib.hashit(self.serialise())
