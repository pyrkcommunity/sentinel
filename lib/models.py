...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, pyrkd):
        golist = pyrkd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_pyrkd(pyrkd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_pyrkd(self, pyrkd, rec):
        import decimal
        import pyrklib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from pyrkd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from pyrkd, with every run
        ...
        # get/create, then sync payment amounts, etc. from pyrkd - pyrkd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(pyrkd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from pyrkd! %s" % e)
            govobj.vote_delete(pyrkd)
            return (govobj, None)
    ...
    def vote_delete(self, pyrkd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(pyrkd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, pyrkd, signal, outcome):
        import pyrklib
        ...
        output = pyrkd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = pyrklib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(pyrkd, signal)

    def sync_network_vote(self, pyrkd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = pyrkd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = PYRKD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 pyrk addr, non-multisig
            if not pyrklib.is_valid_pyrk_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid PYRK address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # PyrkCore restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (pyrklib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = PYRKD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not pyrklib.is_valid_pyrk_address(addr, config.network):
    ...
    def hash(self):
        import pyrklib
        return pyrklib.hashit(self.serialise())
