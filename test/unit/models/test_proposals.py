...
def test_proposal_is_valid(proposal):
    ...
    # this is actually the zocteam multisig address...
    proposal.payment_address = '5AchYc7iQS7ynce7hNZ6Ya8djsbm5N9JBS'
    assert proposal.is_valid() is False
    ...
    proposal.url = 'http://zeroonecentral.org/dean-miller 5493'
    assert proposal.is_valid() is False

    proposal.url = 'http://zeroonecentralisé.org/dean-miller-5493'
    assert proposal.is_valid() is True

    proposal.url = 'http://zeroonecentralisé.org/dean-миллер-5493'
    assert proposal.is_valid() is True
